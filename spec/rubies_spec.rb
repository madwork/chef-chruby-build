...
describe "chruby-build::rubies" do
  ...
  let(:rubies) { JSON.parse File.read("test/integration/rubies_data_bags/data_bags/rubies/ruby-2.3.4.json") }
  ...
  it { expect(chef_run).to create_directory('/opt/rubies/ruby-2.3.4/etc') }
  it { expect(chef_run).to create_template("/opt/rubies/ruby-2.3.4/etc/gemrc") }
